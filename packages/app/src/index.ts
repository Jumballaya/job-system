...
try {
  ...
  section(
    "Fire and forget: the call returns once the backend accepts the work",
  );
  const handle = await add({ amount: 10 });
  console.log(
    `  accepted job ${handle.id.slice(0, 8)}; result read later:`,
    await handle.result(),
  );
  void add({ amount: 100 });
  ...
  section(
    "Scoped dependency: one RequestScope per execution, shared by its Auditor",
  );
  ...
  const labels = await Promise.all(
    ["a", "b", "c", "d"].map((name) =>
      sleep({ ms: 500, label: name }).result(),
    ),
  );
  ...
  section(
    "Retries exhausted: the last failure reaches the caller as JobExecutionError",
  );
  try {
    ...
    console.log(
      `  caught ${error.failure.name}: ${error.failure.message} (job ${error.jobId.slice(0, 8)})`,
    );
  }

  section(
    "NonRetryableError: fails on the first attempt despite the default three",
  );
  try {
  ...
  console.log(
    `  same job: ${one.id === again.id}, different user: ${one.id !== two.id}`,
  );
  console.log("  results:", await Promise.all([again.result(), two.result()]));

  section(
    "Per-job concurrency 1: three users sync one at a time inside a 4-slot worker",
  );
  ...
  section(
    "Cancelled wait: the caller stops waiting, the accepted job still completes",
  );
  ...
  const stray = defineJob({ deps: [], handler: () => "never" });
  try {
